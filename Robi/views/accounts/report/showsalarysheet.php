...
<div class="table-responsive">
	<div class="screen-tree">
		<table class="table table-bordered table-striped" align="center" border="1" width="100%">
			...
			<?
				$i=1;
				/* while($row2=$obj->fetch($sql1))
				{
					$name=$row2->name;
					$tx_designation=$row2->tx_designation;
					$Emp_ID=$row2->emp_id;
			
					$sql3=$obj->select("monthly_salary
where com_name='$id_com_id' and dept_name='$department' and month='$month' and year='$year' and emp_id='$Emp_ID'","gross_salary,basic,house_rent,medical,transport,special,arrear,kpi_add,mobile_bill,pf_add,bonus,ot,gross_payable,total_month_day,present,late,absent,pf_employee,pf_employeer,total_pf_ded,absent_deduction,loan,advance,tax,kpi_de,total_deduction_amount,net_salary,payment_mode ");
					$row3=$obj->fetch($sql3); */
			?>
			<tbody>
				<tr>
					<td><? //echo $i++;?></td>
					<td><? //echo $name;?></td>
					<td><? //echo $tx_designation;?></td>
					<td><? //echo $gross_salary=$row3->gross_salary;?></td>
					<td><? //echo $basic=$row3->basic;?></td>
					<td><? //echo $house_rent=$row3->house_rent;?></td>
					<td><? //echo $medical=$row3->medical;?></td>
					<td><? //echo $transport=$row3->transport;?></td>
					<td><? //echo $special=$row3->special;?></td>
					<td><? //echo $arrear=$row3->arrear;?></td>
					<td><? //echo $kpi_add=$row3->kpi_add;?></td>
					<td><? //echo $mobile_bill=$row3->mobile_bill;?></td>
					<td><? //echo $pf_add=$row3->pf_add;?></td>
					<td><? //echo $bonus=$row3->bonus;?></td>
					<td><? //echo $ot=$row3->ot;?></td>
					<td><? //echo $gross_payable=$row3->gross_payable;?></td>

					<td><? //echo $present=$row3->present;?></td>
					<td><? //echo $late=$row3->late;?></td>
					<td><? //echo $absent=$row3->absent;?></td>
					<td><? //echo $total_pf_ded=$row3->total_pf_ded;?></td>
					<td><? //echo $absent_deduction=$row3->absent_deduction;?></td>
					<td><? //echo $loan=$row3->loan;?></td>
					<td><? //echo $advance=$row3->advance;?></td>
					<td><? //echo $tax=$row3->tax;?></td>
					<td><? //echo $kpi_de=$row3->kpi_de;?></td>
					<td><? //echo $total_deduction_amount=$row3->total_deduction_amount;?></td>
					<td><? //echo $net_salary=$row3->net_salary;?></td>
					<td><? //if($row3->payment_mode == '1'){echo 'Cash';}else if($row3->payment_mode == '2'){echo 'Bank';}?></td>
				</tr>
			...
			<?
				...
				$total_late += $late;
				$total_absent += $absent;
				$total_total_pf_ded += $total_pf_ded;
				$total_absent_deduction += $absent_deduction;
				$total_loan += $loan;
				$total_advance += $advance;
				$total_tax += $tax;
				$total_kpi_de += $kpi_de;
				$final_total_ded += $total_deduction_amount;
			...
			?>
				<tr>
					...
					<td style="text-align:center;"><b>Yearly: </b><? //echo $total_bonus;?></td>
					...
					<td style="text-align:center;"><b>PF DE: </b><? //echo $total_total_pf_ded;?></td>
